...
pre_install_node(){
    ...
    echo
    # Set IsUseWss
    read -p "(是否启用ws协议 1：启用 0：不启用 不填则选择不启用):" IsUseWss
    ...
    echo
    # Set IsUseFspWbSrv
    read -p "(是否启用fsp白板 1：启用 0：不启用 不填则选择不启用):" IsUseFspWbSrv
    [ -z "${IsUseFspWbSrv}" ] 
    echo
    echo "---------------------------"
    ...
    echo
    echo "---------------------------"
    echo "FspAccessAddr = ${FspAccessAddr}"
    echo "---------------------------"
    echo
	# Set FspPort
